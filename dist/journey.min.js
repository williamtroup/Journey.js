...
g.style.display="none";n.body.appendChild(g);z=t("button","close");z.onclick=x;g.appendChild(z);B=t("div","title");g.appendChild(B);C=t("div","description");g.appendChild(C);var b=t("div","buttons");g.appendChild(b);v=t("button","previous");v.onclick=T;b.appendChild(v);A=t("button","next");A.onclick=U;b.appendChild(A);b=f.domElementTypes;for(var e=b.length,h=0;h<e;h++){var u=n.getElementsByTagName(b[h]);u=[].slice.call(u);for(var X=u.length,O=0;O<X;O++){var E=u[O],P=!0;if(l(E)&&E.hasAttribute("data-journey-options")){var d=
E.getAttribute("data-journey-options");if(r(d))if(d=V(d),d.parsed&&D(d.result)){d=d.result;var Y=d=D(d)?d:{};var F=d.order;F=R(F)?F:0;Y.order=F;d.title=w(d.title,null);d.description=w(d.description,null);d.onEnter=N(d.onEnter,null);d.onLeave=N(d.onLeave,null);d.onClose=N(d.onClose,null);d.element=E;R(d.order)&&(r(d.title)||r(d.description))&&(y[d.order]=d,m.push(d.order))}else f.safeMode||(console.error("The attribute 'data-journey-options' is not a valid object."),P=!1);else f.safeMode||(console.error("The attribute 'data-journey-options' has not been set correctly."),
P=!1)}if(!P)break}}m.sort()});l(q.$journey)||(q.$journey=this)})(document,window)})();